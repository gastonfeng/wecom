...
class IrModel(models.Model):
    ...
    is_mail_thread_wecom_message = fields.Boolean(
        string="Mail Thread WeCom Message",
        default=False,
        store=False,
        compute="_compute_is_mail_thread_wecom_message",
        search="_search_is_mail_thread_wecom_message",
        help="Does it support sending messages and notifications through WeCom",
    )

    @api.depends("is_mail_thread")
    def _compute_is_mail_thread_wecom_message(self):
        for model in self:
            if model.is_mail_thread:
                ...
                if any(fname in ModelObject._fields for fname in potential_fields):
                    ...
                    continue
            model.is_mail_thread_wecom_message = False

    def _search_is_mail_thread_wecom_message(self, operator, value):
